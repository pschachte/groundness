% CVS: $Id: peep.pl,v 1.4 1998/10/21 04:26:04 pets Exp $
goal :- comppeepopt(Pil,OptPil,Preds).
...
notmember1(X,[]).
notmember1(X,[F|T]) :-
% this is a numerical comparison!!!   X =\= F,
   notmember1(X,T).
...
peepelim(putnil(R),Dum,RCont,RCont1,Dum2,El) :-
   ...
   El = 1 , RCont1 = RCont .
peepelim(putnil(R),Dum,RCont,RCont1,Dum2,El) :-
   El = 0 , peepelimupd(RCont,R,c(nil),RCont1).
...
*/
peepelim(jumpgt(Dum6,L),Dum,R0,R1,Dum2,0) :- L = abs(1), R1 = R0.
