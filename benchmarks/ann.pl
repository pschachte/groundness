% CVS: $Id: ann.pl,v 1.3 1998/10/21 04:25:40 pets Exp $
%------------------------------------------------------------------------------
...
data([
      clause(f(X,Z), ( p(X,Y), q(Y,Z) )),
      clause(f(X,Y,Z), ( p(X,Y), q(Y,Z) )),
      clause(f(X,Y,Z), ( Y is X+Z, p(X,Y), r(Y), q(Y,Z) )),
      clause(f(X,Y,Z), ( var(X), f(X), q(Y), r(X,Y,Z), q(X) )),
      clause(f(X,Y,Z), ( r(X,Y), p(X,Y), p(W), s(X), q(Y,W,Z) ))
]).
...
add_annotations(Info,Phrase,I,G) :- !,
	...
	para_phrase( Info,Code,Type,Vars,I,G,Flag),
...
para_phrase( and( X,Y ), Conjuncts, Type, BagofVars,I,G,_R) :- !,
	para_phrase(X, Xcode, Xtype, XVars,I,G,F1),
	para_phrase(Y, Ycode, Ytype, YVars,I,G,F2),
	(	Xtype = sequential,!,
...
para_phrase( (N,T,Term),This_term,Type,Vars,_X,_Y,_Z) :-
	(	T == sequential,!,
		Type = T,
		Vars = [],
		This_term = Term
...
% These are used by the file "annotate.pl".
%------------------------------------------------------------
