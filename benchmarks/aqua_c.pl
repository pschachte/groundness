% CVS: $Id: aqua_c.pl,v 1.3 1998/10/21 04:25:42 pets Exp $
...
pred_info('_dummy',0,A).

'_acc_info'(A,B,C,D,E,F,G) :-
        acc_info(A,B,C,D,E,F,G).
'_acc_info'(A,B,C,D,E,F,G) :-
        acc_info(A,B,C,D,E).
'_acc_info'(dcg,A,B,C,B=[A|C],D,[]).

acc_info('_dummy',A,B,C,D,E,F).

acc_info('_dummy',A,B,C,D).

pass_info('_dummy',A).

pass_info('_dummy').

'_replace_default'(A,B,C) :-
        functor(B,A,2),
        '_member'(B,C),
        !.
'_replace_default'(A,B,C) :-
...
end_label(return,A,B) :-
        dcg_clause(A,return,B).
end_label(jump(A),B,C) :-
        dcg_clause(B,label(A),C).
...
translate(c(A,B,C),A=[B|C]) :-
        compile_option(system(cprolog)),
        !.
translate(dcg_clause(A,B,C),A=[B|C]) :-
        compile_option(system(quintus)),
        !.
translate(dcg_clause(A,B,C),A=[B|C]) :-
        compile_option(system(aquarius)),
...
rtest_in_deref(A,B,[pref,A,B|C],C,D,D,E,F) :-
        ...
        !,
        dcg_clause(E,deref(A,B),F).
rtest_in_deref(A,A,[A|B],B,C,C,D,E) :-
        \+write_once,
        !,
        dcg_clause(D,deref(A),E).
...
expand_test(deref(A),A,B,C,D,E,F) :- !,
        dcg_clause(E,deref(B,B),F).
expand_test(trail(A),A,B,C,D,E,F) :- !,
        dcg_clause(E,trail(B),F).
expand_test(trail_if_var(A),A,B,C,D,E,F) :- !,
        tag(var,G),
        dcg_clause(E,test(ne,G,B,H),I),
        dcg_clause(I,trail(B),J),
        dcg_clause(J,label(H),F).
expand_test(nil(A),A,B,C,D,E,F) :- !,
        dcg_clause(E,equal(B,G^[],D),F),
        tag(atom,G).
expand_test(A==B,A,C,D,E,F,G) :-
        atomic(B),
        !,
        atomic_word(B,H),
        dcg_clause(F,equal(C,H,E),G).
...
expand_test(A,B,C,D,E,F,G) :-
        test_tag(A,B,H),
        !,
        dcg_clause(F,test(ne,H,C,E),G).
expand_test('$atom_nonnil'(A),A,B,C,D,E,F) :- !,
        tag(atom,G),
        dcg_clause(E,test(ne,G,B,D),H),
        dcg_clause(H,equal(B,G^[],I),J),
        dcg_clause(J,jump(D),K),
        dcg_clause(K,label(I),F).
expand_test(float(A),A,B,C,D,E,F) :-
        \+float(A),
        !,
        dcg_clause(E,jump(D),F).
expand_test(nonvar(A),A,B,C,D,E,F) :- !,
        dcg_clause(E,test(eq,G,B,D),F),
        tag(var,G).
...
expand_test(functor(A,B,C),A,D,E,F,G,H) :-
        ...
        tag(structure,M),
        dcg_clause(L,pragma(tag(D,M)),N),
        O=1, % align(O),
        dcg_clause(N,pragma(align(D,O)),P),
        dcg_clause(P,equal([D],J^ (B/C),F),H).
expand_test(functor(A,B,C),A,D,E,F,G,H) :-
        atom(B),
        integer(C),
        C==0,
...
expand_test('$name_arity'(A,B,C),A,D,E,F,G,H) :-
        ...
        tag(structure,L),
        dcg_clause(K,pragma(tag(D,L)),M),
        N=1, % align(N),
        dcg_clause(M,pragma(align(D,N)),O),
        dcg_clause(O,equal([D],I^ (B/C),F),H).
expand_test('$name_arity'(A,B,C),A,D,E,F,G,H) :-
        atom(B),
        integer(C),
        C==0,
        !,
        tag(atom,I,G,J),
        dcg_clause(J,equal(D,I^B,F),H).
expand_test('$name_arity'(A,B,C),A,D,E,F,G,H) :-
        number(B),
        integer(C),
        C==0,
        !,
        dcg_clause(G,equal(D,B,F),H).
expand_test(A,B,C,D,E,F,G) :-
        test_to_disj(A,B,H),
...
expand_test_disj(fail,A,B,C,D,E,F) :-
        dcg_clause(E,jump(D),G),
        dcg_clause(G,label(C),F).
expand_test_disj((A;B),C,D,E,F,G,H) :-
        expand_test(A,C,D,I,G,J),
        dcg_clause(J,jump(E),K),
        dcg_clause(K,label(I),L),
...
arith_comparison(A,B,C,true,D,E,F) :- !,
        dcg_clause(E,jump(A,B,C,D),F).
arith_comparison(A,B,C,false,D,E,F) :- !,
        cond(A,G),
        dcg_clause(E,jump(G,B,C,D),F).
...
jump_if(eq,integer(A),B,C,D,E) :-
        ...
        tag(negative,F),
        tag(nonnegative,G),
        dcg_clause(D,test(eq,F,B,C),H),
        dcg_clause(H,test(eq,G,B,C),E).
jump_if(ne,integer(A),B,C,D,E) :-
        fail, % split_integer,
        !,
        tag(negative,F),
        tag(nonnegative,G),
        dcg_clause(D,test(eq,G,B,H),I),
        dcg_clause(I,test(ne,F,B,C),J),
        dcg_clause(J,label(H),E).
...
collect_ptrees([A-B|C],D,E,F,G) :-
        ...
        flat_conj((P ',' Q),R),
        dcg_clause(F,ptree(A,[J|S],(O:-R),[]),T),
...
merge_branch(test(A,B,C,D),[test(A,B,C,E)|F],[],G,H) :-
        dcg_clause(G,test(A,B,C,E),H).
merge_branch(test(ne,A,B,C),[switch(D,B,fail,E,F)|G],[H],I,J) :-
        dcg_clause(I,switch(D,B,H,E,F),K),
        dcg_clause(K,label(H),J),
        tag(var,A).
merge_branch(choice(A/B,C,D),[jump(E)|F],[],G,H) :-
        dcg_clause(G,choice(A/B,C,E),H).
merge_branch(jump(A),[test(B,C,D,E),label(F)|G],[],H,I) :-
	true,
        ...
        % ;   mc68020
        % ),
        dcg_clause(H,test(B,C,D,E),J),
        dcg_clause(J,jump(F),I).
merge_branch(jump(A),[switch(B,C,D,E,F)|G],[],H,I) :-
	true,
        ...
        % ;   mc68020
        % ),
        dcg_clause(H,switch(B,C,D,E,F),I).
merge_branch(jump(A),[deref(B,C),test(D,E,C,F),label(G)|H],[],I,J) :-
	true,
        ...
        % ;   mc68020
        % ),
        dcg_clause(I,deref(B,C),K),
        dcg_clause(K,test(D,E,C,F),L),
        dcg_clause(L,jump(G),J).
merge_branch(jump(A),[deref(B,C),switch(D,C,E,F,G)|H],[],I,J) :-
	true,
        ...
        % ),
        dcg_clause(I,deref(B,C),K),
        dcg_clause(K,switch(D,C,E,F,G),J).
...
cpeep3(call(A),label(B),return,C,D) :-
        dcg_clause(C,jump(A),E),
        dcg_clause(E,label(B),F),
        dcg_clause(F,return,D).

peep3r(test(A,B,C,D),E,test(A,B,C,F),G,H) :-
        no_mod(E,C),
        \+E=label(I),
        dcg_clause(G,test(A,B,C,D),J),
        dcg_clause(J,E,H).
peep3r(A,label(B),A,C,D) :-
        distant_branch(A),
        dcg_clause(C,label(B),E),
        dcg_clause(E,A,D).
peep3r(jump(A,B,C,D),jump(E),label(D),F,G) :-
        cond(A,H),
        lbl(E),
        dcg_clause(F,jump(H,B,C,E),I),
        dcg_clause(I,label(D),G).
peep3r(jump_nt(A,B,C,D),jump(E),label(D),F,G) :-
        cond(A,H),
        lbl(E),
        dcg_clause(F,jump_nt(H,B,C,E),I),
        dcg_clause(I,label(D),G).
peep3r(jump(A,B,C,D),fail,label(D),E,F) :-
        cond(A,G),
        dcg_clause(E,jump(G,B,C,fail),H),
        dcg_clause(H,label(D),F).
peep3r(jump_nt(A,B,C,D),fail,label(D),E,F) :-
        cond(A,G),
        dcg_clause(E,jump_nt(G,B,C,fail),H),
        dcg_clause(H,label(D),F).
peep3r(test(A,B,C,D),fail,label(D),E,F) :-
        eq_ne(A,G),
        dcg_clause(E,test(G,B,C,fail),H),
        dcg_clause(H,label(D),F).
peep3r(test(A,B,C,D),jump(E),label(D),F,G) :-
        eq_ne(A,H),
        lbl(E),
        dcg_clause(F,test(H,B,C,E),I),
        dcg_clause(I,label(D),G).
peep3r(move(A^B,C),adda(B,D,B),move(C,[C]),E,F) :-
        dcg_clause(E,move(A^B,C),G),
        dcg_clause(G,push(C,B,D),F),
        tag(A),
        integer(D),
        reg(C),
        reg(B),
...
peep3r(move(A,B),move(B,C),move(D,B),E,F) :-
        ...
        \+is_in(B,C),
        \+is_in(B,D),
        dcg_clause(E,move(A,C),G),
        dcg_clause(G,move(D,B),F).
...
peep3(pragma(tag(A,B)),move(C,D),move(C,E),F,G) :-
        ...
        \+reg(C),
        \+is_in(E,D),
        dcg_clause(F,move(C,E),H),
        dcg_clause(H,pragma(tag(A,B)),I),
        dcg_clause(I,move(E,D),G).
peep3(move(A,B),pragma(tag(A,C)),move(B,[B]),D,E) :-
        perm(A),
        reg(B),
        dcg_clause(D,move(A,B),F),
        dcg_clause(F,pragma(tag(B,C)),G),
        dcg_clause(G,move(B,[B]),E).
peep3(move(A^r(h),B),push(B,r(h),C),deref(B,D),E,F) :-
        reg(B),
        a_var(D),
        tag(A),
        dcg_clause(E,move(A^r(h),B),G),
        dcg_clause(G,push(B,r(h),C),H),
        dcg_clause(H,move(B,D),F).

peep2r(add(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,add(A,B,C),G),
        tag(integer,D).
peep2r(sub(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,sub(A,B,C),G),
        tag(integer,D).
peep2r(div(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,div(A,B,C),G),
        tag(integer,D).
peep2r(mul(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,mul(A,B,C),G),
        tag(integer,D).
peep2r(pragma(A),pragma(A),B,C) :-
        dcg_clause(B,pragma(A),C).
peep2r(pragma(tag(A,B)),deref(C,D),E,F) :-
        \+ind(C),
        \+ind(D),
        dcg_clause(E,deref(C,D),F).
peep2r(allocate(A),deallocate(A),B,B).
peep2r(jump_nt(A,B,C,D),label(D),E,F) :-
        dcg_clause(E,label(D),F).
peep2r(A,B,C,D) :-
        distant_branch(A),
        \+B=label(E),
        dcg_clause(C,A,D).
...
peep2r(move(A,B),move(B,A),C,D) :-
        \+is_in(B,A),
        dcg_clause(C,move(A,B),D).
peep2r(move(A,B),move(C,B),D,E) :-
        \+is_in(B,C),
        dcg_clause(D,move(C,B),E).
...
peep2r(pair(A,B),C,D,E) :-
        \+C=pair(F,G),
        \+C=label(H),
        dcg_clause(D,pair(A,B),E).
peep2r(choice(1/A,B,C),cut(D),E,F) :-
        dcg_clause(E,cut(D),F).
...
peep2(fadd(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,fadd(A,B,C),H),
        dcg_clause(H,jump(E),G),
        tag(integer,D).
peep2(fsub(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,fsub(A,B,C),H),
        dcg_clause(H,jump(E),G),
        tag(integer,D).
peep2(fdiv(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,fdiv(A,B,C),H),
        dcg_clause(H,jump(E),G),
        tag(integer,D).
peep2(fmul(A,B,C),test(ne,D,C,E),F,G) :-
        dcg_clause(F,fmul(A,B,C),H),
        dcg_clause(H,jump(E),G),
        tag(integer,D).
peep2(test(A,B,C,fail),jump(D),E,F) :-
        lbl(D),
        dcg_clause(E,test(G,B,C,D),H),
        dcg_clause(H,fail,F),
        eq_ne(A,G).
peep2(move(A,B),move(B,C),D,E) :-
        perm(B),
        reg(C),
        dcg_clause(D,move(A,C),F),
        dcg_clause(F,move(C,B),E).
peep2(deref(A,B),move(B,C),D,E) :-
        perm(B),
        reg(C),
        dcg_clause(D,deref(A,C),F),
        dcg_clause(F,move(C,B),E).
peep2(deref(A,B),deref(B,C),D,E) :-
        dcg_clause(D,deref(A,B),F),
        dcg_clause(F,move(B,C),E).
...
peep2(move(A,B),move(B,C),D,E) :-
        ...
        ind(B),
        dcg_clause(D,move(A,B),F),
        dcg_clause(F,move(A,C),E).
peep2(jump(A,B,C,fail),jump(D),E,F) :-
        cond(A,G),
        lbl(D),
        dcg_clause(E,jump(G,B,C,D),H),
        dcg_clause(H,fail,F).
peep2(jump_nt(A,B,C,fail),jump(D),E,F) :-
        cond(A,G),
        lbl(D),
        dcg_clause(E,jump_nt(G,B,C,D),H),
        dcg_clause(H,fail,F).
peep2(switch(A,B,fail,C,C),label(C),D,E) :-
        dcg_clause(D,test(eq,F,B,fail),G),
        dcg_clause(G,label(C),E),
        tag(var,F).
peep2(switch(A,B,C,fail,fail),label(C),D,E) :-
        dcg_clause(D,test(ne,F,B,fail),G),
        dcg_clause(G,label(C),E),
        tag(var,F).
peep2(switch(A,B,fail,C,fail),label(C),D,E) :-
        dcg_clause(D,test(ne,A,B,fail),F),
        dcg_clause(F,label(C),E).
peep2(switch(A,B,C,fail,C),label(C),D,E) :-
        dcg_clause(D,test(eq,A,B,fail),F),
        dcg_clause(F,label(C),E).
peep2(switch(A,B,C,D,C),label(C),E,F) :-
        dcg_clause(E,test(eq,A,B,D),G),
        dcg_clause(G,label(C),F).
peep2(switch(A,B,C,D,C),label(D),E,F) :-
        dcg_clause(E,test(ne,A,B,C),G),
        dcg_clause(G,label(D),F).
peep2(switch(A,B,C,D,D),label(C),E,F) :-
        dcg_clause(E,test(ne,G,B,D),H),
        dcg_clause(H,label(C),F),
        tag(var,G).
peep2(switch(A,B,C,D,D),label(D),E,F) :-
        dcg_clause(E,test(eq,G,B,C),H),
        dcg_clause(H,label(D),F),
        tag(var,G).
peep2(A,adda(B,C,B),D,E) :-
        merge_add,
        dcg_clause(D,adda(B,C,B),F),
        dcg_clause(F,G,E),
        inc_reg(A,B,C,G).
peep2(A,pad(B),C,D) :-
        dcg_clause(C,pad(B),E),
        dcg_clause(E,F,D),
        inc_reg(A,r(h),B,F).
...
peep1(deref(A,A),B,C) :-
        \+write_once,
        dcg_clause(B,deref(A),C).
peep1(move(A,B),C,D) :-
        A\==B,
        s_oper(A,E),
        s_oper(B,F),
        dcg_clause(C,move(E,F),D).
peep1(equal(A,B,C),D,E) :-
        s_oper(A,F),
        s_oper(B,G),
        dcg_clause(D,equal(F,G,C),E).
peep1(unify(A,B,C,D,E),F,G) :-
        s_oper(A,H),
        s_oper(B,I),
        dcg_clause(F,unify(H,I,C,D,E),G).
peep1(jump(fail),A,B) :-
        dcg_clause(A,fail,B).
...
peep1(switch(A,B,C,C,C),D,E) :-
        C\==fail,
        dcg_clause(D,jump(C),E).
peep1(switch(A,B,fail,C,C),D,E) :-
        dcg_clause(D,test(ne,F,B,C),G),
        dcg_clause(G,fail,E),
        tag(var,F).
peep1(switch(A,B,C,fail,fail),D,E) :-
        dcg_clause(D,test(eq,F,B,C),G),
        dcg_clause(G,fail,E),
        tag(var,F).
peep1(switch(A,B,fail,C,fail),D,E) :-
        dcg_clause(D,test(eq,A,B,C),F),
        dcg_clause(F,fail,E).
peep1(switch(A,B,C,fail,C),D,E) :-
        dcg_clause(D,test(ne,A,B,C),F),
        dcg_clause(F,fail,E).
peep1(pad(A),B,C) :-
        D=1, % align(D),
        E is A mod D,
...
peep_flat(switch(unify,A,B,C,D,E),F,G,H) :- !,
        dcg_clause(G,switch(A,B,I,J,E),K),
        dcg_clause(K,label(I),L),
        peep_flat(C,L,M),
        dcg_clause(M,jump(N),O),
        dcg_clause(O,label(J),P),
        peep_flat(D,P,Q),
        dcg_clause(Q,jump(N),R),
        dcg_clause(R,label(N),S),
        peep_flat(F,S,H).
peep_flat(A,B,C,D) :-
...
det_code(A,B,C,D,hash(E),v(F),G,H,I) :- !,
        ...
        dcg_clause(P,hash(E,J,Q,R),S),
        dcg_clause(S,label(O),T),
        insert(U,V,T,W),
        dcg_clause(W,label(R),X),
        dcg_clause(X,pragma(hash_length(Q)),Y),
        insert(Z,A1,Y,B1),
...
det_code(A,B,C,D,switch(E),v(F),G,H,I) :-
        ...
        dcg_clause(M,switch(J,K,N,O,P),Q),
        type_test(E,F,R),
        disj_test(var(F),R,S),
        not_test(S,T),
...
det_code(A,B,C,D,equal(structure,E/F),v(G),H,I,J) :- !,
        ...
        P=1, % align(P),
        dcg_clause(O,pragma(align(K,P)),Q),
        dcg_clause(Q,equal([K],N^ (E/F),R),S),
...
insert_uninit([A|B],C,D,[entry(E,F)|G],H,I,J) :-
        diffv(A,D,K),
        dcg_clause(I,label(E),L),
        init_code_block(K,C,L,M),
        dcg_clause(M,jump(H),N),
...
uninit_allowbag(A,B,B,C,D,E,F) :-
        functor(A,G,H),
        my_member(stree(G/H,(I:-J),(K:-L),M,N,O),B),
        !,
        copy((K:-L),(A:-P)),
        uninit_bag(P,C,Q),
        copy((I:-J),(A:-R)),
