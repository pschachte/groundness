% CVS: $Id: nand.pl,v 1.3 1998/10/21 04:26:03 pets Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if_then_else_1(K, J, Tk, Fi, Tk2) :-
	(K = J ->
		set_union(Tk, Fi, Tk2);
		Tk2 = Tk).

if_then_else_2(K, I, Tk2, Fj, Tk3) :-
	(K = I ->
 		set_union(Tk2, Fj, Tk3);
		Tk3 = Tk2).

if_then_else_3(K, IPi, ISi, Tk3, V, Tko, Tk3) :-
	((set_member(K, IPi); set_member(K, ISi)) ->
		set_union(Tk3, [V], Tko);
		Tko = Tk3).

if_then_else_4(K, I, Fk, V, Fko, Fk) :-
	(K = I ->
                set_union(Fk, [V], Fko);
		Fko = Fk).

if_then_else_5(K, Pi, I, CIk, SjJ, CIk2) :-
	((set_member(K, Pi); K = I) ->
		set_difference(CIk, SjJ, CIk2);
		CIk2 = CIk).

if_then_else_6(I, CIk, V, CIk2, CIk3) :-
	((set_member(I, CIk), set_member(V, Fk)) ->
			set_difference(CIk2, [I], CIk3);
			CIk3 = CIk2).

if_then_else_7(K, I, CIk3, Gs, V, CIk4) :-
	(K = I ->
                exclude_if_vector_in_false_set(CIk3, Gs, V, CIk4);
		CIk4 = CIk3).

if_then_else_8(K, J, CIk4, I, CIko) :-
	(K = J ->
		set_difference(CIk4, [I], CIko);
		CIko = CIk4).

if_then_else_9(K, J, IPk, I, IPko) :-
	(K = J ->
                set_union(IPk, [I], IPko);
                IPko = IPk).

if_then_else_10(K, I, ISk, J, ISko) :-
	(K = I ->
                set_union(ISk, [J], ISko);
		ISko = ISk).

if_then_else_11(K, SjJ, Pk, PiI, Pko) :-
	(set_member(K, SjJ) ->
		set_union(Pk, PiI, Pko);
		Pko = Pk).

if_then_else_12(K, PiI, Sk, SjJ, Sko) :-
	(set_member(K, PiI) ->
