% CVS: $Id: strips.pl,v 1.2 1998/10/19 04:22:32 pets Exp $
% Artificial Intelligence Techniques in Prolog by Yoav Shoham, 1993
...
%	not(state_constrains(Cond, C)) ,not(state_constrains(C, Cond)) ,
...
display_regression(un, X , un, Op1) :- % Execute upon backtracking
...
state_constrains(holding(X), on(X,_)).
state_constrains(holding(X), on(_,X)).
state_constrains(holding(X), ontable(X)).
state_constrains(holding(_), handempty).
state_constrains(holding(X), holding(Y)) :- not(X=Y).
state_constrains(on(X,Y), on(X,Z)) :- not(Z=Y).
state_constrains(on(Y,X), on(Z,X)) :- not(Z=Y).
state_constrains(clear(X), on(_,X)).
state_constrains(clear(X), holding(X)).
state_constrains(ontable(X), on(X,_)).
...
init_state6([clear(c),ontable(a),on(c,a),ontable(b), clear(b),handempty]).
goal6([on(a,b),on(b,c)]).

init_state7([clear(a),clear(b),clear(c),ontable(a),ontable(b),ontable(c),handempty]).
goal7([on(b,c),on(c,a),on(a,b) ]).

init_state8([clear(b),on(b,a), on(a,d), on(d,c), ontable(c),handempty]).
goal8([on(a,b),ontable(b), on(c,d),ontable(d)]).

init_state9([clear(a),on(a, b), on(b,d), on(d,c), ontable(c),handempty]).
goal9([on(a,b),ontable(b), on(c,d),ontable(d)]).
...
test7 :-
	init_state7(InitState),
	goal7(Goal),
	do_it(Goal, InitState).
test8 :-
	init_state8(InitState),
	goal8(Goal),
	do_it(Goal, InitState).
test9 :-
	init_state9(InitState),
	goal9(Goal),
	do_it(Goal, InitState).
